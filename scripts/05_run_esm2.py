"""
ESM-2 Multilabel Training Script for GPU Cluster
...
print("\n" + "="*80)
print("ESM-2 MULTILABEL TRAINING PIPELINE")
print("="*80)
...
class ESM2MultilabelDataset(Dataset):
    """PyTorch Dataset for ESM-2 multilabel classification.
    ESM-2 tokenises raw sequences directly (no spacing needed).
    """
...
class ESM2MultilabelClassifier(nn.Module):
    """ESM-2 encoder with a multilabel classification head.

    Available ESM-2 variants (comment specifies hidden dim):
        facebook/esm2_t6_8M_UR50D    ->  320
        facebook/esm2_t12_35M_UR50D  ->  480
        facebook/esm2_t30_150M_UR50D ->  640
        facebook/esm2_t33_650M_UR50D -> 1280  (best, slowest)
    """

    def __init__(self, model_name="facebook/esm2_t6_8M_UR50D",
                 n_labels=N_LABELS, dropout=0.3, freeze_esm=False):
        super().__init__()
...
def train_esm2_multilabel(train_df, val_df, test_df, dataset_name,
                           model_name="facebook/esm2_t6_8M_UR50D",
                           batch_size=4, epochs=5, learning_rate=2e-5,
                           weight_decay=0.01,
                           freeze_esm=False, use_scheduler=True):
    print(f"\n{'='*60}")
    print(f"Training ESM-2 Multilabel: {dataset_name}")
    print(f"{'='*60}")
    ...
    train_dataset = ESM2MultilabelDataset(train_df, tokenizer, max_length=256)
    val_dataset   = ESM2MultilabelDataset(val_df,   tokenizer, max_length=256)
    test_dataset  = ESM2MultilabelDataset(test_df,  tokenizer, max_length=256)
    ...
    print("\nInitialising ESM-2 multilabel classifier...")
    model = ESM2MultilabelClassifier(model_name=model_name, n_labels=N_LABELS,
                                      dropout=0.3, freeze_esm=freeze_esm)
...
def plot_training_history(history, dataset_name):
    ...
    plt.tight_layout()
    save_path = os.path.join(RESULTS_DIR, f'esm2_multilabel_{dataset_name.lower()}_training2.png')
...
if __name__ == "__main__":
    ...
    # ── dbAMP ────────────────────────────────────────────────────────────────
    print("\n" + "="*80)
    print("TRAINING ESM-2 ON dbAMP DATASET")
    print("="*80 + "\n")

    dbamp_model, dbamp_history, dbamp_results = train_esm2_multilabel(
    ...
    dbamp_model_path = os.path.join(RESULTS_DIR, 'esm2_multilabel_dbamp_final2.pth')
    ...
    # ── DRAMP ────────────────────────────────────────────────────────────────
    print("\n" + "="*80)
    print("TRAINING ESM-2 ON DRAMP DATASET")
    print("="*80 + "\n")

    dramp_model, dramp_history, dramp_results = train_esm2_multilabel(
    ...
    dramp_model_path = os.path.join(RESULTS_DIR, 'esm2_multilabel_dramp_final2.pth')
    ...
    results_path = os.path.join(RESULTS_DIR, 'results_esm2_multilabel2.csv')
    ...
    plt.suptitle('ESM-2 Multilabel Comparison', fontsize=14, fontweight='bold')
    plt.tight_layout()
    comparison_path = os.path.join(RESULTS_DIR, 'esm2_multilabel_comparison2.png')
